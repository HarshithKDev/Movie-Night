...
app.use(helmet({
    contentSecurityPolicy: {
      directives: {
        ...
        "connect-src": ["'self'", "http://localhost:3000", "ws://localhost:3000", "https://movienight-backend-veka.onrender.com", "wss://movienight-backend-veka.onrender.com", "https://securetoken.googleapis.com", "https://identitytoolkit.googleapis.com", "https://www.gstatic.com", "https://*.agora.io", "wss://*.agora.io:*", "https://*.sd-rtn.com", "wss://*.sd-rtn.com:*"],
      },
    },
}));
  ...
  try {
    ...
    wss.on('connection', async (ws, req) => {
        try {
            ...
            const { token, roomCode } = query;

            if (!token || !roomCode || !/^[A-Z0-9]{6}$/.test(roomCode)) return ws.terminate();
            
            const room = await roomsCollection.findOne({ roomCode });
            if (!room) return ws.terminate();

            const user = await admin.auth().verifyIdToken(token);
            ws.isHost = user.uid === room.hostId;
            ...
            ws.on('message', async (message) => {
                ...
                try {
                    ...
                    const allowedTypes = ['play', 'pause', 'seek', 'sync'];
                    if (!allowedTypes.includes(data.type)) return;

                    if (data.type === 'sync') {
                        newState = { isPlaying: data.isPlaying, currentTime: data.time };
                    } else {
                        newState = { isPlaying: data.type === 'play', currentTime: data.time };
                    }
                    
                    await roomsCollection.updateOne({ roomCode: ws.roomCode }, { $set: { "state.isPlaying": newState.isPlaying, "state.currentTime": newState.currentTime, "state.lastUpdated": Date.now() } });
    ...
    app.put('/api/movies/:movieId', authenticateUser, [param('movieId').isMongoId(), body('newName').notEmpty().trim().escape()], handleValidationErrors, async (req, res) => { 
        try { 
    ...
    app.post('/api/generate-upload-url', authenticateUser, [body('fileName').notEmpty().trim().escape(), body('fileType').equals('video/mp4')], handleValidationErrors, async (req, res) => { 
        try { 
    ...
    app.get('/api/movies/:movieId/stream-url', authenticateUser, [param('movieId').isMongoId()], handleValidationErrors, async (req, res) => { 
        try { 
            const { movieId } = req.params; 
    ...
    app.delete('/api/movies/:movieId', authenticateUser, [param('movieId').isMongoId()], handleValidationErrors, async (req, res) => { 
        try { 
            const { movieId } = req.params; 
    ...
    app.post('/api/rooms', authenticateUser, [body('roomCode').isLength({ min: 6, max: 6 }).isAlphanumeric().trim(), body('fileId').isURL(), body('fileName').notEmpty().trim().escape(), body('filePath').notEmpty().trim()], handleValidationErrors, async (req, res) => { 
        ...
        const newRoom = { roomCode, movieId: movie._id, hostId: req.user.uid, createdAt: new Date(), state: { isPlaying: false, currentTime: 0, lastUpdated: Date.now() } }; 
        await roomsCollection.insertOne(newRoom); 
        res.status(201).json(newRoom); 
    });
    
    app.get('/api/rooms/:roomCode', [param('roomCode').isLength({ min: 6, max: 6 }).isAlphanumeric().trim()], handleValidationErrors, async (req, res) => { 
        const { roomCode } = req.params; 
        const room = await roomsCollection.findOne({ roomCode }); 
        if (room) { 
            res.status(200).json({ movieId: room.movieId, hostId: room.hostId }); 
        } else { 
